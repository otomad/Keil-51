...
*/
#define LENGTH 20
char RecvString_buf[LENGTH]; //定义数据包长度为19个字符
...
unsigned char getDutyNum() {
	bit modified = 0;
	unsigned char i;
	unsigned long int num = 0;
	for (i = 11; i <= 13; i++) { //表明占空比的数据在第 11~13 位
		const char ch = RecvString_buf[i];
		if (ch == '\0') break;
		if (ch >= '0' && ch <= '9') {
			modified = 1;
			num = num * 10 + (ch - '0');
	...
	if (num > 100) num = 50;
	return (modified ? num : 50);
}
bit Deal_UART_RecData() { //处理串口接收数据包函数（成功处理数据包则返回1，否则返回0）
	buff_strlwr(); //将输入的内容全部转换为小写，这样用户输入大小写都是可以的了
//	PutString(RecvString_buf);
	if (!CompareCMD_head("output ")) return 0;
	if (CompareCMD_tail(7, 3, "sin")) setWaveType(SINE);
	else if (CompareCMD_tail(7, 3, "saw")) setWaveType(SAW);
	else if (CompareCMD_tail(7, 3, "tri")) setWaveType(TRI);
	else if (CompareCMD_tail(7, 3, "pwm") || CompareCMD_tail(7, 3, "sqr") || CompareCMD_tail(7, 3, "squ")) {
		duty = getDutyNum();
		setWaveType(SQR);
	}
	else return 0;
