 ...
 * @brief  返回值=X的Y次方
 */
long int LCD_Pow(long int X, long int Y) {
	unsigned char i;
	long int Result = 1;
	for (i = 0; i < Y; i++) {
		Result *= X;
 ...
 * @retval 无
 */
void LCD_ShowNum(unsigned char Line, unsigned char Column, unsigned long int Number,
...
/*void LCD_ShowInputPin(unsigned int pin_) {
	char str[5] = "";
	unsigned int i, len = 4, pin = pin_;
	for (i = len - 1; i >= 0; i++) {
		unsigned int num = pin % 10;
		pin /= 10;
		str[i] = '0' + num;
	}
	for (i = 0; i < len; i++) {
